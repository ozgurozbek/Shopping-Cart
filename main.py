...
def checkout():
    if 'email' not in session:
        ...
        with sqlite3.connect('database.db') as conn:
            cur = conn.cursor()
            try:
                cur.execute("INSERT INTO checkout SELECT * FROM kart;")
                conn.commit()
                msg = "Added successfully"
            except:
                conn.rollback()
                msg = "Error occured"
        conn.close()
    clearCart() #Remove all items from the cart AFTER adding them to a seperate database section.
    return redirect(url_for('root'))
...
def profileOrders():
    #This page needs attention after DB changes
    if 'email' not in session:
        return redirect(url_for('root'))
    loggedIn, firstName, noOfItems = getLoginDetails()
    with sqlite3.connect('database.db') as conn:
        ...
        userId = cur.fetchone()
        cur.execute("SELECT productId FROM checkout WHERE userId = ?", (userId))
        productId = cur.fetchall()
    conn.close()
    return render_template("boughtItems.html", productId=productId , userId=userId)
